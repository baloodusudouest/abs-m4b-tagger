...
def find_orphans(roots: list, known_paths: set, unmatched_dir: str,
                 min_age_seconds: int) -> list:
    """Fichiers audio du disque inconnus d'Audiobookshelf."""
    known = {os.path.normpath(p) for p in known_paths}
    now = time.time()
    orphans = []
    for path in scan_audio_files(roots, exclude=[unmatched_dir] if unmatched_dir else []):
